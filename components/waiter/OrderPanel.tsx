"use client";

import { useState } from "react";
import { useRouter } from "next/navigation";
import {
  ArrowLeft,
  Send,
  Receipt,
  StickyNote,
  ShieldAlert,
  Minus,
  Plus,
  Trash2,
  CheckCircle2,
} from "lucide-react";
import Card from "@/components/ui/Card";
import Badge from "@/components/ui/Badge";
import Button from "@/components/ui/Button";
import ConfirmDialog from "@/components/ui/ConfirmDialog";
import MenuPicker from "./MenuPicker";
import OrderSummary from "./OrderSummary";
import { KitchenStatus, Order, RestaurantTable } from "@/lib/types";
import { useRestaurant } from "@/context/RestaurantContext";
import { useToast } from "@/context/ToastContext";
import { useAuth } from "@/context/AuthContext";
import { formatCurrency } from "@/lib/utils";

interface CartLine {
  key: string;
  foodId: string;
  name: string;
  price: number;
  quantity: number;
  notes?: string;
}

const STATUS_TONE: Record<KitchenStatus, "yellow" | "blue" | "green" | "neutral"> = {
  Pending: "yellow",
  Preparing: "blue",
  Ready: "green",
  Served: "neutral",
};

// Lower number = shown first — mirrors the kitchen ticket ordering.
const STATUS_PRIORITY: Record<KitchenStatus, number> = {
  Pending: 0,
  Preparing: 1,
  Ready: 2,
  Served: 3,
};

export default function OrderPanel({ table, order }: { table: RestaurantTable; order?: Order }) {
  const { menu, startSession, sendItemsToKitchen, requestBill, setCustomerNotes, updateKitchenItemStatus } =
    useRestaurant();
  const { user } = useAuth();
  const { showToast } = useToast();
  const router = useRouter();

  const [cart, setCart] = useState<CartLine[]>([]);
  const [notesDraft, setNotesDraft] = useState(order?.customerNotes ?? "");
  const [billConfirmOpen, setBillConfirmOpen] = useState(false);

  function addToCart(item: { foodId: string; quantity: number; notes?: string }) {
    const food = menu.find((f) => f.id === item.foodId);
    if (!food) return;
    setCart((prev) => [
      ...prev,
      {
        key: `${item.foodId}_${Date.now()}`,
        foodId: item.foodId,
        name: food.name,
        price: food.price,
        quantity: item.quantity,
        notes: item.notes,
      },
    ]);
  }

  function updateCartQty(key: string, qty: number) {
    setCart((prev) => prev.map((c) => (c.key === key ? { ...c, quantity: Math.max(1, qty) } : c)));
  }

  function removeCartLine(key: string) {
    setCart((prev) => prev.filter((c) => c.key !== key));
  }

  function handleSendToKitchen() {
    if (cart.length === 0 || !user) return;
    // The table only becomes "Occupied" and the order only exists in the system
    // once real items are actually sent — not just from opening the table screen.
    const orderId = order?.id ?? startSession(table.id, user.id, user.name).id;
    sendItemsToKitchen(
      orderId,
      cart.map((c) => ({ foodId: c.foodId, quantity: c.quantity, notes: c.notes }))
    );
    showToast(`${cart.length} item${cart.length > 1 ? "s" : ""} sent to the kitchen.`, "success");
    setCart([]);
  }

  function handleRequestBill() {
    if (!order) return;
    requestBill(order.id);
    showToast("Bill requested. Admin will generate the invoice.", "info");
  }

  function handleSaveNotes() {
    if (!order) return;
    setCustomerNotes(order.id, notesDraft);
    showToast("Customer notes saved.", "success");
  }

  function handleMarkServed(itemId: string) {
    if (!order) return;
    updateKitchenItemStatus(order.id, itemId, "Served");
    showToast("Item marked as served.", "success");
  }

  const cartTotal = cart.reduce((s, c) => s + c.price * c.quantity, 0);
  const sortedItems = order
    ? [...order.items].sort((a, b) => STATUS_PRIORITY[a.status] - STATUS_PRIORITY[b.status])
    : [];

  return (
    <div className="space-y-4 pt-2">
      <button
        onClick={() => router.push("/waiter/dashboard")}
        className="flex items-center gap-1.5 text-sm font-medium text-ink/55 hover:text-spice-600 focus-ring rounded"
      >
        <ArrowLeft className="h-4 w-4" /> Back to floor
      </button>

      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="font-display text-2xl font-semibold text-ink">Table {table.number}</h2>
          <p className="text-xs text-ink/45">Seats {table.capacity} · {table.status}</p>
        </div>
        {order && (
          <Badge tone={order.status === "BillRequested" ? "yellow" : "spice"} dot>
            {order.status === "BillRequested" ? "Bill Requested" : "Order Open"}
          </Badge>
        )}
      </div>

      {!order && (
        <p className="rounded-xl bg-spice-50/70 px-4 py-2.5 text-xs font-medium text-ink/50">
          This table is still <span className="font-semibold text-leaf-600">Available</span> — it only becomes
          Occupied once you send an order to the kitchen.
        </p>
      )}

      <div className="grid grid-cols-1 gap-4 lg:grid-cols-5">
        {/* Menu picker */}
        <Card className="lg:col-span-3 animate-fade-in">
          <h3 className="mb-3 font-display text-base font-semibold text-ink">Add Items</h3>
          <MenuPicker menu={menu} onAdd={addToCart} />
        </Card>

        {/* Order + cart */}
        <div className="space-y-4 lg:col-span-2">
          <Card className="animate-fade-in">
            <h3 className="mb-3 font-display text-base font-semibold text-ink">New Items to Send</h3>
            {cart.length === 0 ? (
              <p className="py-6 text-center text-xs text-ink/40">Add dishes from the menu to build this round.</p>
            ) : (
              <div className="space-y-2">
                {cart.map((line) => (
                  <div key={line.key} className="rounded-xl border border-spice-100 bg-white/60 p-2.5">
                    <div className="flex items-center justify-between gap-2">
                      <div className="min-w-0">
                        <p className="truncate text-sm font-medium text-ink">{line.name}</p>
                        {line.notes && <p className="truncate text-[11px] text-ink/45">{line.notes}</p>}
                      </div>
                      <div className="flex shrink-0 items-center gap-1.5">
                        <button
                          onClick={() => updateCartQty(line.key, line.quantity - 1)}
                          className="flex h-6 w-6 items-center justify-center rounded-md border border-spice-200 text-ink/60 hover:bg-spice-100 focus-ring"
                        >
                          <Minus className="h-3 w-3" />
                        </button>
                        <span className="w-5 text-center text-xs font-semibold">{line.quantity}</span>
                        <button
                          onClick={() => updateCartQty(line.key, line.quantity + 1)}
                          className="flex h-6 w-6 items-center justify-center rounded-md border border-spice-200 text-ink/60 hover:bg-spice-100 focus-ring"
                        >
                          <Plus className="h-3 w-3" />
                        </button>
                        <button
                          onClick={() => removeCartLine(line.key)}
                          className="ml-1 flex h-6 w-6 items-center justify-center rounded-md text-maroon-600 hover:bg-maroon-600/10 focus-ring"
                        >
                          <Trash2 className="h-3 w-3" />
                        </button>
                      </div>
                    </div>
                  </div>
                ))}
                <div className="flex items-center justify-between pt-1 text-sm">
                  <span className="text-ink/50">Round subtotal</span>
                  <span className="font-semibold text-ink">{formatCurrency(cartTotal)}</span>
                </div>
                <Button className="w-full" icon={<Send className="h-4 w-4" />} onClick={handleSendToKitchen}>
                  Send Order to Kitchen
                </Button>
              </div>
            )}
          </Card>

          {order && order.items.length > 0 && (
            <Card className="animate-fade-in">
              <h3 className="mb-3 font-display text-base font-semibold text-ink">Order Status</h3>
              <div className="max-h-64 space-y-2 overflow-y-auto pr-1">
                {sortedItems.map((it) => (
                  <div key={it.id} className="rounded-xl border border-spice-100 bg-white/50 p-2.5">
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-sm text-ink/70">
                        {it.quantity}× {it.name}
                      </span>
                      <Badge tone={STATUS_TONE[it.status]} dot>
                        {it.status}
                      </Badge>
                    </div>
                    {it.status === "Ready" && (
                      <Button
                        size="sm"
                        variant="outline"
                        className="mt-2 w-full"
                        icon={<CheckCircle2 className="h-3.5 w-3.5" />}
                        onClick={() => handleMarkServed(it.id)}
                      >
                        Mark Served
                      </Button>
                    )}
                  </div>
                ))}
              </div>
            </Card>
          )}

          {order && (
            <Card className="animate-fade-in">
              <h3 className="mb-3 flex items-center gap-1.5 font-display text-base font-semibold text-ink">
                <StickyNote className="h-4 w-4" /> Customer Notes
              </h3>
              <textarea
                value={notesDraft}
                onChange={(e) => setNotesDraft(e.target.value)}
                placeholder="Allergies, seating preference, celebration notes..."
                rows={3}
                className="w-full rounded-xl border border-spice-200 bg-white/70 p-3 text-sm text-ink placeholder:text-ink/40 focus-ring focus:border-spice-400"
              />
              <Button size="sm" variant="outline" className="mt-2" onClick={handleSaveNotes}>
                Save Notes
              </Button>
            </Card>
          )}

          {order && order.items.length > 0 && (
            <Card className="animate-fade-in space-y-3">
              <OrderSummary items={order.items} />
              {order.status === "BillRequested" ? (
                <p className="flex items-center gap-2 rounded-xl bg-saffron-500/10 p-3 text-xs font-medium text-saffron-600">
                  <ShieldAlert className="h-4 w-4 shrink-0" />
                  Bill requested — only Admin can generate the final invoice.
                </p>
              ) : (
                <Button
                  className="w-full"
                  variant="secondary"
                  icon={<Receipt className="h-4 w-4" />}
                  onClick={() => setBillConfirmOpen(true)}
                >
                  Request Bill
                </Button>
              )}
            </Card>
          )}
        </div>
      </div>

      <ConfirmDialog
        open={billConfirmOpen}
        onClose={() => setBillConfirmOpen(false)}
        title="Request bill"
        description="This notifies the admin that Table is ready for billing. Only Admin can generate the final invoice. Continue?"
        confirmLabel="Request Bill"
        onConfirm={handleRequestBill}
      />
    </div>
  );
}
